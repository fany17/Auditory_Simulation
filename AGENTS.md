...
`doc/` 保存本项目自己的边界、当前任务状态、正式规范和历史规划材料。本项目不维护覆盖 `Auditory_Simulation` 与 `stn-rhythm-pilot` 的跨项目总文档。
...
- `doc/CURRENT_TASK.md` 当前为 `PLANNING_ONLY`，方向是 `M6A-PUBLIC`。
- 第一候选任务是 `M6A-PUBLIC-001`：冻结 OpenNeuro `ds004703` 版本、许可、文件与时间边界，并设计冻结预训练表征的最小逐层对齐基线。
- wav2vec 2.0、HuBERT、WavLM 是首轮候选；从零训练或微调声音基座必须另立任务，不能作为默认启动步骤。
- STN 患者实验、同步、原始 STN-LFP 与患者特异分析不属于本项目。
- 旧总纲 v2.0 与旧章程 v0.4-draft 已降级为历史材料，不再定义当前任务优先级。
- 当前没有经审核接受的 M6A 正式代码、环境、数据、模型或实验结果；`test/` 中已有材料不改变上述状态。

在此状态下，不得自动启动 Agent B、安装环境、下载数据或模型、运行大规模实验、声明任务完成或进入下一编号任务。
...
创建并执行 `M6A-PUBLIC-001` 前至少需要人工确认：
...
- 用户已授权在本项目目录整理和编写文件，但这不等于授权初始化 Git。
- 未经用户明确指定 Git 管理方式，不得运行 `git init`，不得建立远端、提交或推送。
- 不得擅自切换分支、过滤提交、重置工作区、删除 `.git` 或批量恢复覆盖。
- 如需验证 Git 操作，应先在 sibling 临时镜像中试验，并保护原始资料。
- 如果用户以后明确指定本目录为唯一 Git 工作区，应先只读盘点、核对远程历史并建立可恢复保护；随后直接在本目录持续工作，不得为每个任务重复建立镜像。
- 临时镜像验证结束后，按用户要求收敛，并通过回收站移除。
...
- 未经任务书和人工授权，不安装或修改环境，不下载大型模型、数据或权重。
- 外部模型、代码和数据必须记录论文或官方来源、版本、许可证、下载日期、哈希、输入输出和资源需求。
