# M6A-PUBLIC-001 Audio context 与 final embargo 候选报告

| 字段 | 内容 |
|---|---|
| 日期 | 2026-08-13 |
| 状态 | `FINAL_EMBARGO_CANDIDATE_AWAITING_COORDINATOR_REVIEW` |
...
- `final_embargo=max(2.0, 0.5, 1.091796875, 0.0, 0.0006349206349206349)=2.0 s`。真实轻量 319 行 split 在 2.0 s 候选下通过：train/validation/test=223/48/48，固定 block assignment，English=319、Catalan=0。
...
可声称：单一模型的非密码学镜像缓存、配置/权重安全读取、单 passage 输入隔离、有限支持重采样、synthetic hidden-state/frame-time 行为、0.0 s 跨 split 输入 overlap 候选及 2.0 s final embargo 候选已有机器证据。

不可声称：第三方镜像或 `main` 得到密码学固定；Transformer 具有局部 receptive field；baseline-final 已接受；真实 acoustic/wav2vec2 特征、真实 neural target、encoding baseline、G3、M6A→M6B exchange candidate、整条 M6A accepted/frozen 或任何科学结果已经完成。
...
- `baseline_final=false`、`neural_extraction_allowed=false`、`exchange_candidate_exists=false`、`scientific_result_claimed=false`；模型下载权限已关闭。
- 当前停止在 `FINAL_EMBARGO_CANDIDATE_AWAITING_COORDINATOR_REVIEW`，等待协调者独立复核；未运行正式特征提取、神经 target、baseline 或 G3 smoke。

## Git 与下一节点

本报告生成时工作树待提交推送；最终 Git 状态将在阶段回传中记录。下一节点尚未获授权；协调接受本候选前不继续执行。
