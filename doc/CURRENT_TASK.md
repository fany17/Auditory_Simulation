...
当前节点为 `G2_CANDIDATE_AWAITING_COORDINATOR_REVIEW`：

- G2 full dataset、neural metadata、PyBIDS、split 与单一 promotion gate 已形成轻量候选证据；
- 该状态不是 G2 PASS，必须等待协调者独立验收；
- 在协调验收前，不下载 audio model、不读取真实神经波形生成 target、不运行 baseline；
- 后续 audio cross-split context、final embargo 与 baseline-final split guard 仍是独立门禁。
...
- 数据下载/读取：保留首次 374/377 与 Range 切换历史；2026-08-13 仅补齐 SD011/SD019 的 2 个缺失区间，复用已有 350 chunks，0 失败，并原子装配 SD011/SD019/SD022。当前 full audit 为 377/377 paths、14,173,350,514 bytes，missing/unexpected/byte mismatch/partial 均为 0；状态仅为 `G2_CANDIDATE_AWAITING_COORDINATOR_REVIEW`，不是 G2 PASS；
...
- 剩余执行门禁：G2 协调验收、audio cross-split context 测量、final embargo 与 baseline-final split guard；方法冻结和 G2 candidate 均不等于 G3 启动、真实 target、M6A exchange candidate、整条 M6A accepted/frozen 或科学结果；
...
- M6A→M6B exchange contract review：`REVISED_DRAFT_ACCEPTED_FOR_CANDIDATE_PREPARATION`；consumer 为 `READY_WAITING_M6A_CANDIDATE`。真实 exchange candidate 尚无、consumer cross-test 未运行，contract 未 accepted/frozen；candidate 准备仍受 G2 协调验收、final embargo/split guard 和真实 method/runtime/canary 门禁约束。
- G2 promotion gate：真实机器入口为 `G2_CANDIDATE_AWAITING_COORDINATOR_REVIEW`，所有 required checks 为 true；`g2_pass_claimed=false`、`candidate_contains_raw_data=false`。首次因额外要求 C-prefix 名称唯一而 fail closed 的报告已保留；实际协调要求的 eligible 名单唯一性与 1346/727 聚合均通过。
- 最新联合验证：2203 专用环境为 `91 passed, 133 subtests`，Ruff 通过，mypy 对 29 个源文件通过；neural target semantic gate、main config gate 与 formal-src direct-convolution scan 均通过。旧 checkpoint 仍保留于历史报告。
