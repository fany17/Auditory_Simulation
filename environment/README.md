...
- 开发工具：`environment/dev-requirements_2203.txt`，仅安装到本项目环境，用于 pytest、Ruff 与 mypy 审计
...
`types-jsonschema` 与运行时 `jsonschema 4.25.1` 主版本/次版本匹配；本项目不对 jsonschema 全局关闭 mypy。MNE 与 SoundFile 当前未提供本项目可直接采用的类型声明，因此只在两处延迟导入上使用精确的 `import-untyped` 抑制，不影响其余模块检查。
